...
class ColorFormatter(logging.Formatter):
    def format(self, record):
        ...
        if record.levelno >= logging.ERROR:
            self._style._fmt = (
                f"{RED}[%(levelname)s] %(message)s (%(filename)s:%(lineno)d){RESET}"
            )
        else:
...
def save_graph(data: pd.DataFrame):
    ...
    plt.plot(data["km"], data["price"], marker="o", linestyle=" ", color="blue")
...
def main():
    ...
    try:
        ...
        print(GREEN)
        logging.info(
            f"Le prix estimé pour une voiture qui a {KM:_} km au compteur : {predict:.2f} €{RESET}"
        )
